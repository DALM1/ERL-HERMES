        ...
        <<"POST">> ->
            ...
            case safe_decode_json(Body) of
                {ok, Map} ->
                    ...
                    case erl_messenger_user:login(Username, Password) of
                        {ok, U} -> json_reply(200, U, Req1);
                        {error, bad_credentials} -> json_reply(401, #{error => <<"bad_credentials">>}, Req1);
                        {error, not_found} -> json_reply(404, #{error => <<"not_found">>}, Req1);
